...
var Dijkstra = new Class({
               ...
               initialize: function(start_point, end_point, r) {
                   ...
                   this.Road = createMatrix(this.nodes + 1, this.nodes + 1);

                   this.R = createArray( this.nodes + 1);

                   this.F = createArray( this.nodes + 1);

                   this.S = createArray( this.nodes + 1);
               ...
               _read: function() {
                      ...
                      var n = this.matrix.length, x, y, cost;

                      for(var i = 1; i <= this.nodes; i++) {

                           for(var j = 1; j <= this.nodes; j++) {

                              if(i == j) this.Road[i][j] = 0; 

                                  else   
                                         this.Road[i][j] = this.PInfinit; 
                           }
                      }
               ...
               _solve: function() {
                       ...
                       for(var i = 1; i <= this.nodes; i++) {
                           ...
                           this.R[ i ] = this.Road[ this.start ][ i ];

                          if( this.start != i ) 

                              if( this.R[ i ] < this.PInfinit) 

                                  this.F[ i ] = this.start
                       }

                 //execute n-1 times
                 for(var i=1; i <= this.nodes-1;i++) {
     
                      min = this.PInfinit

                      for(var k = 1; k <= this.nodes; k++) {
                    ...
                    for(var j = 1; j <= this.nodes; j++) {
                        ...
                        if( this.S[ j ] == 0 ) 
                           ...
                           if(this.R[ j ] > this.R[ posMin ] + this.Road[ posMin ][ j ]) {

                                     this.R[ j ] = this.R[ posMin ] + this.Road[ posMin ][ j ]
               ...
               getRoad: function( node ) {
                        ...
                        if(this.F[ node ]) this.getRoad( this.F[ node ] )
